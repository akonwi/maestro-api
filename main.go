...
var teams map[string]string = map[string]string{}

func init() {
	teams["Manchester United"] = "19538871"
	teams["Wolverhampton Wanderers"] = "8cec06e1"
	teams["Ipswich Town"] = "b74092de"
	teams["Tottenham Hotspurs"] = "361ca564"
	teams["Aston Villa"] = "8602292d"
}
...
func main() {
	...
	for name, id := range teams {
		fmt.Printf("Calculating for %s\n", name)
		req, err := http.NewRequest("GET", "https://fbrapi.com/teams?team_id="+id, nil)
		if err != nil {
			panic(fmt.Errorf("Error creating request for %s: %s", name, err))
		}
